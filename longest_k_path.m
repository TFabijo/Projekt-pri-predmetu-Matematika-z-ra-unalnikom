function [max_path, max_length] = longest_k_path(graph, k)
    ...
    %vij
    %W [i, h] je uteˇz oziroma dolˇzina
    %najvecje h-poti P (h, vi), ki se konča pri vi in smo jo doslej odkrili
    W = zeros(n, k);
    X = cell(n, k);
    X(:,:) = {'*'};% Initialize with '*'
    ...
    for i = 1:n
        for j = i+1:n %% lohko gremo od i+1 ker mamo topološko urejeno
